...
public class DownLoadFrame extends JFrame{
	...
	private List<Progressar> progressarList= Lists.newArrayList();
	private Map<Resource, Progressar> maps= Maps.newHashMap();
	private Progressar progressar;
	private Resource resource;
	private String size="";
	private DownloadPanel downloadPanel;
	public DownLoadFrame(Configuration parse) {
		setName("版本更新("+Configuration.serverUrl+")");
		setType(Frame.Type.UTILITY);
		this.list =parse.resources;
		initListener();
		...
		setUndecorated(true);
		setSize(400, 300);
		this.downloadPanel = new DownloadPanel(this,parse,maps);
...
//		setAlwaysOnTop(true);
	...
	public  void inputstreamtofile(InputStream ins, File file)
		...
		if(y!=lengths) throw new Exception("下载失败！");
	...
	public boolean downloadUrlFile(int i, String url, String path)
			throws Exception {
		try {
			resource = list.get(i);
	...
	public void updateResources(Configuration parse) {
		this.list =parse.resources;
